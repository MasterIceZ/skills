...
cafe-grader has no test groups, so the validator checks the FULL constraint set only. Subtask-specific bounds are the generators' responsibility (checked by the budget review), because every test, whatever subtask it was designed for, has to pass this one validator.
...
int main(int argc, char* argv[]) {
    ...
    // Read input exactly as the problem specifies, against the full constraints.
