...
`Wrong answer` and `Time limit exceeded` are pure tags: each promises the solution fails in only that one way. Partial solutions frequently break the promise. An O(N·M) solution that also reads values into `int` TLEs on the max-size groups *and* wrong-answers the small tests carrying out-of-range values. That file is neither `Wrong answer` nor `Time limit exceeded`; it is `Incorrect`.
...
| File | Polygon tag | Observed mix | Expected cafe score |
|------|-------------|--------------|----------------|
| `solutions/sol.cpp` | Main correct solution | OK on all | 100 |
| `solutions/sol_st1_2.cpp` | Incorrect | OK st1–2, WA st3, TLE st4 | sum of the scores of the tests it passes |
| … | | | |

The tag and the score are separate facts: under per-test scoring a wrong solution keeps the points of every test its bug doesn't reach, so its score says nothing about which tag Polygon will accept. Record both. The same table feeds the Solutions section of `UPLOAD.md`.

These tags matter only to Polygon. cafe-grader has no notion of solution types; there the expected score is what counts.
